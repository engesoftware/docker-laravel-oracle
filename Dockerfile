...
RUN apt-get update && apt-get install -y apt-utils git unzip libbz2-dev libzip-dev zip libaio-dev libxml2-dev && apt-get clean -y
...
RUN curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer && chmod +x /usr/local/bin/composer \
     ...
     && docker-php-ext-install soap zip bz2 \
     && pecl install xdebug \
